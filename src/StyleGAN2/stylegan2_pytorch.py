...
def mixed_list(n, layers, latent_dim, device):
    #tt is random number [0,1) * layers, so number 1-5
...
#         x = style_vectorizer(z)
#         result.append((x, num_layers))
...
class StyleGan2Trainer(Trainer):
    ...
    def load_config(self):
        # Call the parent class's load_config to get the initial config
        super().load_config()
        
        # Load additional config values specific to ChildClass
        config = self.config() if not self.config_path.exists() else json.loads(self.config_path.read_text())
        
        # Merge with parent class's config if necessary
    ...
    def config(self):
        # Get the parent's config first
        parent_config = super().config()

        # Merge it with the child's config
        child_config = {
            'image_size': self.image_size, 
    ...
    def train_epoch(self):
        ...
        for i, data in enumerate(tqdm(self.dataset)):
            try:
                ...
                print(f"Failed during batch {i}, continuing...\nError: {e}") 
                continue
